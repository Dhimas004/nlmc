...
<section id="testimoni" class="container mb-5">
    <div class="container card shadow py-3">
        <h4 class="font-rubik bg-primary w-25 two-values py-2 px-5 text-white" style="border-radius: 2px 40px; text-indent: 4px;">Testimoni</h4>

        <!-- Owl Carousel -->
        <div class="owl-carousel owl-theme">
            <?php
            $list_testimoni = mysqli_query($koneksi, "SELECT * FROM tb_testimoni ORDER BY rand()");
            ...
            ?>
                <div class="item py-2 bg-white m-3 img-fluid">
                    <div class="product font-rale card p-5 card-item text-center" style="min-height: 450px; max-height: 450px; position: relative;">
                        <img class="img-list-cover d-block mx-auto" src="assets/testimoni/<?= htmlspecialchars($testimoni['gambar_produk']); ?>" alt="<?= htmlspecialchars($testimoni['gambar_produk']); ?>" style="max-width: 150px; max-height: 150px; border-radius: 10px;">
