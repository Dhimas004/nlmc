...
// ====================== FUNCTION ======================
function setAlert($title = '', $text = '', $type = '', $buttons = '', $show = false)
{
...
function tambahProduk($data)
{
    ...
    if (!isset($_FILES['gambar_produk']) || $_FILES['gambar_produk']['error'] !== UPLOAD_ERR_OK) {
        ...
        echo "<script>alert('Gambar Produk Tidak Ada')</script>";
        return false;
    }
    ...
    if (!isset($file) || $file['error'] !== UPLOAD_ERR_OK) {
        // File tidak berhasil di-upload, beri tahu pengguna atau lakukan tindakan yang sesuai
        echo "<script>alert('File Produk Tidak Ada')</script>";
        return false;
    }
    ...
    $upload_path_file = '../assets/file/' . $filename;
    if (!move_uploaded_file($_FILES['gambar_produk']['tmp_name'], $upload_path)) {
        // Gagal memindahkan file, beri tahu pengguna atau lakukan tindakan yang sesuai
        echo "<script>alert('Gagal Memindahkan Gambar Produk')</script>";
        return false;
    }
    if (!move_uploaded_file($file['tmp_name'], $upload_path_file)) {
        // Gagal memindahkan file, beri tahu pengguna atau lakukan tindakan yang sesuai
        echo "<script>alert('Gagal Memindahkan File Produk')</script>";
        return false;
    }

    // Insert into tb_produk table
    $query = "INSERT INTO tb_produk
              VALUES ('','$brand', '$nama_produk', '$harga', '$deskripsi', '$nama_gambar', '$id_kategori', '$id_user','$filename')";
...
function ubahProduk($data)
{
    ...
    if (isset($file) && $file['name'] != '') {
        $last_file = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM tb_produk WHERE id_produk = '$id_produk'"))['file'];
        unlink('../assets/file/' . $last_file);
        if (!move_uploaded_file($file['tmp_name'], $upload_path_file)) {
            // Gagal memindahkan file, beri tahu pengguna atau lakukan tindakan yang sesuai
            return false;
        }
        mysqli_query($koneksi, "UPDATE tb_produk SET `file` = '$filename' WHERE id_produk = '$id_produk'");
    }

    // Check if the cover image was uploaded successfully
    if (!isset($_FILES['gambar_produk']) || $_FILES['gambar_produk']['error'] !== UPLOAD_ERR_OK && !isset($file) || $file['error'] !== UPLOAD_ERR_OK) {
        // Jika tidak ada gambar baru diunggah, lakukan pembaruan data produk tanpa mengubah gambar
        $query = "UPDATE tb_produk SET brand = '$brand', nama_produk = '$nama_produk', harga = '$harga', deskripsi = '$deskripsi', id_kategori = '$id_kategori' WHERE id_produk = '$id_produk'";
        mysqli_query($koneksi, $query);
    ...
    $query = "UPDATE tb_produk SET brand = '$brand', nama_produk = '$nama_produk', harga = '$harga', deskripsi = '$deskripsi', gambar_produk = '$nama_gambar', id_kategori = '$id_kategori' WHERE id_produk = '$id_produk'";
    mysqli_query($koneksi, $query);
...
class Product
{
    ...
    public function getData($table = 'tb_produk')
    {
        ...
        if ($table == 'tb_cart') {
            $query .= " AND id_user = '{$_SESSION['id_user']}'";
        }
...
function tambahTestimoni($data)
{
    ...
    // Move uploaded file to desired location
    $upload_path = '../assets/testimoni/' . $nama_gambar;
    if (!move_uploaded_file($_FILES['gambar_produk']['tmp_name'], $upload_path)) {
        // Gagal memindahkan file, beri tahu pengguna atau lakukan tindakan yang sesuai
        echo "<script>alert('Gagal Memindahkan Gambar Produk')</script>";
        return false;
...
function ubahTestimoni($data)
{
    ...
    if (!isset($_FILES['gambar_produk']) || $_FILES['gambar_produk']['error'] !== UPLOAD_ERR_OK) {
        // Jika tidak ada gambar baru diunggah, lakukan pembaruan data produk tanpa mengubah gambar
        $query = "UPDATE tb_testimoni SET deskripsi = '$deskripsi', id_user = '$id_user' WHERE id_testimoni = '$id_testimoni'";
        mysqli_query($koneksi, $query);
        // Record the action in the history
        return mysqli_affected_rows($koneksi);
    }
    ...
    // Get the current image file name
    $query_get_gambar = mysqli_query($koneksi, "SELECT gambar_produk FROM tb_testimoni WHERE id_testimoni = '$id_testimoni'");
    $row = mysqli_fetch_assoc($query_get_gambar);
    $gambar_produk_lama = $row['gambar_produk'];

    // Update tb_testimoni table with new data including the new image file name
    $query = "UPDATE tb_testimoni SET deskripsi = '$deskripsi', gambar_produk = '$nama_gambar', id_user = '$id_user' WHERE id_testimoni = '$id_testimoni'";
    mysqli_query($koneksi, $query);

    // Delete the old image file if the update was successful
    if (mysqli_affected_rows($koneksi) > 0) {
        unlink('../assets/testimoni/' . $gambar_produk_lama);
    }
...
{
    // Periksa apakah nomor HP diawali dengan "0"
    if (substr($nomor_hp, 0, 1) === '0') {
        // Ubah "0" pertama menjadi "62"
...
class Cart
{
    ...
    public function checkout($no_hp, $id_user, $nama_pelanggan, $alamat_pelanggan, $metode_pembayaran, $product_id = null, $product_qty = 1)
    {
        $no_hp = ubahNomorHP($no_hp);
        ...
        $query_penjualan = "INSERT INTO tb_penjualan (nama_pelanggan, deskripsi, alamat_pelanggan, metode_pembayaran, total_pembayaran, tanggal_penjualan,no_hp) VALUES ('$nama_pelanggan', '$deskripsi_produk', '$alamat_pelanggan', '$metode_pembayaran', '$total_pembayaran', '$tanggal_penjualan','$no_hp')";
